...
class LiveExecutor:
    ...
    def _check_orphan_position(self):
        ...
        if pos_size > 0:
            ...
            if not self.broker.flatten():
                self.alerts.error(
                    f"EMERGENCY: Orphan flatten failed. {pos_size} contracts may remain open. "
                    "Manual intervention required IMMEDIATELY.")
                self._halt('orphan_flatten_failed')
                return
            ...
            try:
                ...
                if not exit_ids.get('stop') or not exit_ids.get('target'):
                    ...
                    if not self.broker.flatten():
                        self.alerts.error(
                            ...
                            "Manual intervention required IMMEDIATELY.")
                        self._emergency_close_record(t, 'flatten_failed')
                        self._halt('flatten_failed_bracket_incomplete')
                        return
                    self._reset_trade()
                    return
            except Exception as e:
                ...
                if not self.broker.flatten():
                    self.alerts.error(
                        ...
                        "Manual intervention required IMMEDIATELY.")
                    self._emergency_close_record(t, 'flatten_failed')
                    self._halt('flatten_failed_bracket_exception')
                    return
                self._reset_trade()
                return
    ...
    def _emergency_close_record(self, t: LiveTrade, reason: str):
        ...
        trade_record = {
            ...
            'total_r': round(est_r, 3), 'pnl_usd': round(est_pnl, 2),
            'reason': reason,
        ...
        if not self.shadow:
            if not self.broker.flatten():
                self.alerts.error(
                    ...
                    "Manual intervention required IMMEDIATELY.")
                self._emergency_close_record(t, 'flatten_failed')
                self._halt('flatten_failed_close_trade')
                return
